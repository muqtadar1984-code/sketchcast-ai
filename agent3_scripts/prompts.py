...
def build_episode_prompt(
    ...
    if scene_block:
        ...
        tail = tail.replace(
            ...
            'individual segments.\n'
            'Layer group names are VISIBLE PARTS of the picture — a shape, a '
            'body, a zone a viewer could point at in unlabelled art — never a '
            'process or relation ("melting"). Name an arrow by its two ends '
            '(solid_to_liquid_arrow) and say in the asset text where each '
            'named part sits.\n'
