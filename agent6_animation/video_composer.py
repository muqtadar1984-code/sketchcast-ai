...
def compose_episode_videos(
    ...
    if _scene_flag():
        ...
        # few threads hydrated the same face concurrently, and a thread that
        # saw another's half-written asset.png regenerated a DIFFERENT face
        # for its segment. Warm once, then every segment finds the file.
        #
        # allow_generate=False, like the hand above. Hydration is NOT gated on
        # it (visual_library_integration pulls the roster face onto disk
        # before it calls through), so a face the library already has is still
        # on disk before the pool — which is the whole point of warming. What
        # the flag stops is PAYING here: the cast names a student face for
        # every lesson, but only a lesson whose script actually gives a
        # segment two-voice dialogue ever puts that student on the board, so
        # generating it up front would bill and block on a face the lesson may
        # never draw. A face that has to be generated is generated by the
        # first segment that genuinely needs it, under the per-key asset lock
        # (so it is still generated once, and once only).
            ...
            try:
                ...
                for _k in _face_keys:
                    if _resolve_face(_k) is None:
                        logger.info("cast face %s is not in the library; the first "
                                    "segment that draws it will generate it", _k)
            except Exception as exc:  # noqa: BLE001
